...
SetSMSessionContext create_sm_pdu_session_v4(
  ...
  req_rat_specific->mutable_default_ambr()->set_max_bandwidth_ul(state_ambr.br_ul);
  req_rat_specific->mutable_default_ambr()->set_max_bandwidth_dl(state_ambr.br_dl);

  // For UT
  // state_ambr.br_unit = 1;

  req_rat_specific->mutable_default_ambr()->set_br_unit(
      static_cast<magma::lte::AggregatedMaximumBitrate::BitrateUnitsAMBR>(state_ambr.br_unit));
